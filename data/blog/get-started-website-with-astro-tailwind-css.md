...
<!-- /added codeblock wrapped with triple tick marks and just language following! -->
  ...
  files.forEach(file => {
      ...
      while (currentNode != null) {
          if (currentNode.textContent != null) {
              // gather raw text value from every HTML element in htmlString
              searchIndexReference.push({ id: id++, text: currentNode.textContent });
              // get element's ID if it exists
              // if it does not exist, set the ID back in the original HTML string
          }
